...
def run_metadata(args):
    if args.ceres_multi_imu_init == "kalibr-style":
        multi_imu_policy = (
            "one Kalibr-style initializer and one Ceres joint solve"
        )
    else:
        multi_imu_policy = (
            "one explicit Kalibr-result warm start and one Ceres joint solve"
        )
    metadata = {
        "ceres_mode": args.ceres_mode,
        "ceres_platform": args.ceres_platform if args.ceres_mode == "docker" else "native",
        "ceres_default_policy": (
            "corner-defaults topology; multi-IMU runner uses "
            f"{multi_imu_policy}"
        ),
...
def ceres_corner_command(args, dataset: pathlib.Path, run_dir: pathlib.Path,
    ...
    use_kalibr_style_init = (
        multi_imu and args.ceres_multi_imu_init == "kalibr-style"
    )
    ...
    if multi_imu:
        if kalibr_result_path:
            ...
            if not use_kalibr_style_init:
                command.append("--estimate-camera-translation-prior")
...
def run_benchmark_case(args, name, dataset: pathlib.Path, out_root: pathlib.Path,
    ...
    if (multi_imu and args.ceres_multi_imu_init == "kalibr"
            and init_result is None):
        raise RuntimeError(
            "--ceres-multi-imu-init kalibr requires an existing Kalibr "
            "result; run Kalibr first or provide --reuse-kalibr-from. "
            "It never falls back silently to a no-Kalibr initializer."
        )

    def run_ceres(candidate_dir):
        print(f"[{name}/{label}] Ceres default", flush=True)
        c_cmd = ceres_corner_command(
            args, dataset, candidate_dir, imu_data_names,
            init_result
        )
        ...
        return result_path, row

    ceres_result, ceres_row = run_ceres(ceres_dir)
...
def parse_args():
    ...
    parser.add_argument("--ceres-multi-imu-init",
                        choices=["kalibr-style", "kalibr"],
                        default="kalibr-style",
                        help="Initialization source for multi-IMU Ceres runs. kalibr-style is the production no-Kalibr path: one deterministic initializer followed by one joint solve; kalibr is an explicit warm-start diagnostic.")
