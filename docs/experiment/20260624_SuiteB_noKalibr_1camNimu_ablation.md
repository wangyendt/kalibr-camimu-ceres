...
- 新增 camera-IMU time-shift prior 修复：旧实现对正值 gyro norm 做未去均值、未归一化 full-lag 点积，天然偏向 0 lag。改为去均值归一化相关后，b09 的 chain-prior 初值从 `0 ms` 改成 `+4.007 ms`，与 Kalibr joint `+3.842 ms` 只差 `0.166 ms`。但 b01 会在 full search 下选到 `+122 ms` 假峰，而 Kalibr joint 是 `-116.9 ms`；因此默认新增 `0.05 s` search window 与 `boundary_peak_rejected` gate，边界峰回退到 0，避免直接接受危险初值。
...
| chain-prior + normalized time shift + default staged | 否 | camera time-shift prior 改为 normalized correlation；b09 接受 `+4.007ms` 非边界峰 | 77.2 | 1.063 | +0.095 | 0.868 / 4.81 | time offset 修复成功，但 camera translation/rotation basin 基本未变 |
...
- b01 是 normalized time-shift 的反例：full normalized correlation 会找到 `+122 ms`，而 Kalibr/Ceres single/joint 的实际解在 `-117 ms` 附近；限制到 `0.05 s` 又会卡在边界。当前代码用 `boundary_peak_rejected` 回退到 0，只作为防坏门限。大 offset 数据仍需要 Ceres-single seed 或多候选 residual gate，不能让 chain-prior cold start 单独承担。
...
| `out/docker_benchmarks/b09_time_shift_normcorr_boundary_reject_init_20260625` | b09 normalized time-shift + boundary gate init-only，接受 `+4.007ms` |
| `out/docker_benchmarks/b01_time_shift_normcorr_boundary_reject_init_20260625` | b01 normalized time-shift + boundary gate init-only，拒绝 `+50ms` 边界峰 |
