...
> **关于绝对量级。** 上表三个 Ceres 行都在 `0.31 px` 附近，而本文档「验证证据」一节记录的是 `0.229823 px`。二者相差不是回归：这次复跑使用当时 `--corner-defaults` 配合 `--ceres-multi-imu-candidate-preset none` 的单候选路径，与原记录命令不完全相同；该 runner 参数后来已随生产单路径收口而删除。本节要说明的是**同一条命令下只换 loss** 的相对差异，四行之间是严格可比的；跨文档比绝对值则不可比。判断有无回归请看表内三行的相对关系：新 `rho0` 在三项残差上都优于旧 `rho0`，外参和 time shift 差也更小。
