...
这样 Ceres 在构造线性系统时只看到 Kalibr 同样的一阶权重，不会额外引入标准 `ceres::CauchyLoss` / `ceres::HuberLoss` 中的 $\rho''$ 项。源码桥接如下：

| 对象 | 位置 | 数学含义 |
|---|---|---|
| `kalibrMEstimatorWeight(...)` | `src/optimizer/calibration_problem.cpp` | 计算本节的 $w(s)$ |
| `kalibrMEstimatorRho(...)` | `src/optimizer/calibration_problem.cpp` | 返回 $\{w(s)s,\;w(s),\;0\}$ |
| `KalibrMEstimatorLoss::Evaluate(...)` | `src/optimizer/calibration_problem.cpp` | 把 Kalibr IRLS 权重接入 Ceres `LossFunction` |

因此，本书后续说“Cauchy width 10 已对齐”时，包含两层含义：第一，policy 类型和参数对应 Kalibr；第二，Ceres 侧使用的是 Kalibr IRLS 线性化语义，而不是 Ceres 标准 Cauchy 的 $\rho''$ 语义。
