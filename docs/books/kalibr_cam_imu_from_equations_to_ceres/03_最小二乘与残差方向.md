...
但真正传给 error term 的还不是这个 $\boldsymbol\Omega$。Kalibr 在构造 IMU residual 时，会再乘一个**噪声缩放因子**：

```python
# IccSensors.py，addGyroscopeErrorTerms / addAccelerometerErrorTerms
weight = 1.0 / gyroNoiseScale
gerr = ket.EuclideanError(im.omega, im.omegaInvR * weight, w + b_i)
```

也就是说，实际进入代价函数的 information matrix 是：

$$
\boldsymbol\Omega_\omega
=
\frac{1}{\kappa_\omega\,\sigma_{\omega,d}^2}\mathbf I_3,
\qquad
\boldsymbol\Omega_a
=
\frac{1}{\kappa_a\,\sigma_{a,d}^2}\mathbf I_3,
$$

其中 $\kappa_\omega=\texttt{gyroNoiseScale}$、$\kappa_a=\texttt{accelNoiseScale}$。它们等价于把 IMU 噪声标准差整体放大 $\sqrt\kappa$ 倍：

$$
\sigma_{\omega,d}
\longmapsto
\sqrt{\kappa_\omega}\,\sigma_{\omega,d}.
$$

这个因子的实际用途是**手动调整相机项和 IMU 项的相对权重**。IMU YAML 里标称的 noise density 往往偏乐观（真实器件还有未建模的温漂、振动耦合、量化噪声），直接使用会让 IMU 项过度主导，把 B 样条轨迹拉得过硬，反而压低重投影残差的话语权。取 $\kappa_\omega,\kappa_a>1$ 就是在说"IMU 没有 YAML 声称的那么准"。

需要注意两点。第一，$\kappa$ 是 `IccCalibrator.optimize(...)` 的关键字参数，**默认值为 $1$**，而 cam-imu 的命令行入口 `kalibr_calibrate_imu_camera` 并没有暴露对应的开关；只有以 Python API 方式调用 `IccCalibrator` 时才能改。所以走标准命令行流程时 $\kappa_\omega=\kappa_a=1$，上式退化回 $\boldsymbol\Omega=\boldsymbol\Sigma^{-1}$。第二，$\kappa$ 只作用在**主 IMU 的 gyro / accel measurement 项**上；bias motion prior 和多 IMU 路径中的 `GyroscopeError` 传的是未加权的 `im.omegaInvR`。

因此在与 Kalibr 做数值对拍时，如果对方是通过 Python API 设了非 $1$ 的 noise scale，而你这边按 YAML 直接取 $\boldsymbol\Sigma^{-1}$，两边的 IMU 项权重会整体差一个常数倍——参数估计不会完全一致，且 cost 数值必然对不上。
...
### 3.9.4 在 Ceres 里复刻 Kalibr IRLS 语义

把 Kalibr 的 M-estimator 迁移到 Ceres 时，不能只看名字。Ceres 标准 `ceres::CauchyLoss`、`ceres::HuberLoss` 和 Kalibr 的 `MEstimator` 都会让大 residual 降权，但它们传给线性化系统的二阶信息不同。

Ceres 的 `LossFunction::Evaluate(s, rho)` 接口把 robust loss 写成三项：
...
Ceres 的 trust-region problem builder 会利用 $\rho_1$ 和 $\rho_2$ 来修正 residual/Jacobian 对 normal equation 的贡献。这是 Ceres 对一般 robust loss 的标准二阶处理。

Kalibr 的 aslam_backend 不走这条语义。前面已经从 `ErrorTerm` 的 weighted error / weighted Jacobian 看到，Kalibr 对当前 residual 只取一个权重 $w(s)$，然后在本轮线性系统中把：

$$
\bar{\mathbf e}
\longmapsto
\sqrt{w(s)}\,\bar{\mathbf e},
\qquad
\bar{\mathbf J}
\longmapsto
\sqrt{w(s)}\,\bar{\mathbf J}.
$$

这等价于把这一项的 normal equation 贡献写成：
...
其中 $w(s_k)$ 在一次线性化中视为常数。也就是说，Kalibr 使用的是 IRLS 形式，而不是把 $w(s)$ 的导数继续展开进 Hessian。

要把这个语义搬进 Ceres，关键是认清 Ceres 对 $\rho_0,\rho_1,\rho_2$ 三项的用途并不相同：

- $\rho_1$ 和 $\rho_2$ 只进入线性化，决定这一项对 $\mathbf H$、$\mathbf b$ 的贡献；
- $\rho_0$ 不进入线性化，但它是 Ceres 报告的 cost，也是 trust-region 判据里 actual reduction 的来源（predicted reduction 则由 $\rho_1,\rho_2$ 构造的模型给出）。

所以 IRLS 语义只强制了两件事：$\rho_1(s)=w(s)$，以及 $\rho_2(s)=0$。$\rho_0$ 还有选择余地，而这个余地必须用来保证一件事——

$$
\rho_1(s)
=
\frac{\mathrm d\rho_0}{\mathrm ds}(s).
$$

这是 Ceres 内部默认成立的关系。一旦破坏它，trust-region 的 actual reduction 和 predicted reduction 就来自两个互不相干的函数，步长接受判据会失真。

反面例子是最容易想到的写法 $\rho_0(s)=w(s)\,s$。以 Cauchy 为例，它给出
...
\frac{c^2 s}{c^2+s},
\qquad
\frac{\mathrm d\rho_0}{\mathrm ds}
=
\frac{c^4}{(c^2+s)^2},
$$

而声明的 $\rho_1=w(s)=\dfrac{c^2}{c^2+s}$。两者差一个因子 $w(s)$，在 $s\gg c^2$ 时可以差出一个数量级。因此 $\rho_0=w(s)s$ 不能用。

正确的做法是反过来：先固定 $\rho_1=w$，再把 $\rho_0$ 取成 $w$ 的原函数

$$
\rho_0(s)
=
\int_0^s w(u)\,\mathrm du,
\qquad
\rho_1(s)=w(s),
\qquad
\rho_2(s)=0.
...
$\rho_1=\mathrm d\rho_0/\mathrm ds$ 于是自动成立。$\rho_2=0$ 这一条要说清楚它的性质：**它不是 $\rho$ 的真实二阶导，而是一个刻意的曲率近似**。Kalibr 的 IRLS 把权重当成本轮的常数，只做 Gauss-Newton，不把 $\mathrm dw/\mathrm ds$ 写进 Hessian；置零就是在 Ceres 的接口里把这件事说出来。真实的 $\rho''$ 对 Cauchy 是 $-\rho_1^2/c^2<0$，并不为零。本章几个 policy 的原函数都有闭式：

| policy | $w(s)$ | $\rho_0(s)=\int_0^s w$ |
|---|---|---|
| none | $1$ | $s$ |
| Cauchy | $\dfrac{1}{1+s/c^2}$ | $c^2\ln\!\left(1+\dfrac{s}{c^2}\right)$ |
| Huber | $1$ 或 $k/\sqrt s$ | $s\ (s\le k^2)$；$2k\sqrt s-k^2\ (s>k^2)$ |
| Geman-McClure | $\dfrac{c^2}{(c^2+s)^2}$ | $\dfrac{s}{c^2+s}$ |
| Blake-Zisserman | $\dfrac{e^{-s}}{e^{-s}+\epsilon}$ | $\ln\dfrac{1+\epsilon}{e^{-s}+\epsilon}$ |

（Huber 的两段在 $s=k^2$ 处取值同为 $k^2$，函数连续。）

这里有一个值得注意的巧合：Cauchy 那一行的 $\rho_0=c^2\ln(1+s/c^2)$ 正是 `ceres::CauchyLoss(a)` 的 $\rho$（其中 $a^2=c^2$），Huber 那一行也正是 `ceres::HuberLoss(a)` 的 $\rho$（其中 $a=k$）。这不是偶然——Kalibr 的 $w(s)$ 本来就等于这两个标准 loss 的 $\rho'$。换句话说：

> Kalibr 和 Ceres 标准 loss 的**降权规则完全相同**。表面上的区别只剩 $\rho''$：Ceres 标准实现给出真实的二阶项（Cauchy 为 $\rho''=-\rho_1^2/a^2$），我们把它置零。

但这个"区别"其实也不存在，这一点必须讲透，否则很容易高估自定义 loss 的必要性。Ceres 用 `Corrector` 类实现鲁棒化，它的构造函数开头有一条短路：
...
Triggs 修正**只在 $\rho''>0$ 时启用**。而 Cauchy、Huber 以及 3.9.2 表里其余几个 policy 全都是凹的，$\rho''\le0$ 恒成立，这条短路每次都命中。注意这里要写 $\le$ 而不是 $<$：Huber 在阈值内 $\rho_0(s)=s$ 是仿射的，$\rho''$ 恰好等于零，不是负数——它同样满足 `rho[2] <= 0.0`，所以短路照样命中。也就是说，即便老老实实交出真实的 $\rho''$，Ceres 也会把它丢掉，退回到"$\sqrt{\rho_1}$ 同时缩放残差和 Jacobian"——**这正是 Kalibr 的 IRLS**。

> **所以对 Cauchy 和 Huber 而言，$\rho_2=0$ 与 $\rho_2=\rho''$ 给出相同的结果。** 本机 Ceres 2.1 上用一维最小算例实测：`CauchyLoss(3.0)` 与同 $\rho_0,\rho_1$ 但 $\rho_2$ 置零的自定义 loss，收敛点 `x=5.0000000000035181`、迭代数 $4$，逐位一致。

把这一条和上面的"巧合"合起来，结论是：**本节构造出来的 loss，与 `ceres::CauchyLoss(c)`、`ceres::HuberLoss(k)` 在 Ceres 当前的求解路径上数值等价**，不是近似等价。在 `2025_03_14_00_10_18` 的 Kalibr 热启动、staged `pbg,pbegt` 口径下，二者的 reprojection 均值实测同为 `0.309145562193544 px`。

这句话的适用范围值得说清楚，它由两截拼成，强度并不一样。前一截是 $\rho_0,\rho_1$ 解析相同，这是纯数学，`tests/test_math.cpp` 里对着 `ceres::CauchyLoss` / `ceres::HuberLoss` 逐点比对，已经锁死。后一截是 `Corrector` 对 $\rho''\le0$ 短路，这是 Ceres 的实现细节而非它承诺的接口语义；本书的实测是在 Ceres 2.1 上做的，而本仓库的 CMake 并没有把 Ceres 版本钉死。单测能保证的是"我们没有把 $\rho_0$ 改坏"，不是"未来任何一版 Ceres 上二者都等价"。

那还要不要自定义？要，但理由和直觉里的不一样：

1. **表达意图。** 置零把"这里做的是 IRLS，不是完整 Newton"写进代码，读者不必去翻 `Corrector` 的短路才能确认。
2. **留接口。** Geman-McClure、Blake-Zisserman 这些 Kalibr 用到而 Ceres 没有内置的权重函数，只能走这条路。
3. **凸损失才是分水岭。** 如果将来引入 $\rho''>0$ 的 policy，短路不再命中，置零与不置零就会产生真实差异——那时这个选择才有数值后果。

反过来也要记住：**不能用"Ceres 标准 loss 的 $\rho''$ 语义不同"去解释任何一次数值不一致。** 对凹损失这个解释是错的，真正的差异只可能来自 $w(s)$ 本身、宽度换算，或者鲁棒核之外的地方。宽度换算这一条尤其容易出错：Kalibr 的 `CauchyMEstimator(sigma2)` 里 `sigma2` 是**平方误差**的分母（`MEstimatorPolicies.cpp:46`，`getWeight(error) = 1/(1+error/_sigma2)`），而 `ceres::CauchyLoss(a)` 的参数是 $a$ 且内部用 $a^2$，所以对应关系是 $a=\sqrt{\texttt{sigma2}}$，差一个平方。

源码桥接如下：

| 对象 | 位置 | 数学含义 |
|---|---|---|
| `kalibrMEstimatorWeight(...)` | `src/optimizer/calibration_problem.cpp` | 计算本节的 $w(s)$ |
| `kalibrMEstimatorRho(...)` | `src/optimizer/calibration_problem.cpp` | 返回 $\{\int_0^s w,\;w(s),\;0\}$ |
| `KalibrMEstimatorLoss::Evaluate(...)` | `src/optimizer/calibration_problem.cpp` | 把 Kalibr IRLS 权重接入 Ceres `LossFunction` |

最后要提醒一处**不可对齐**的地方：$\rho_0$ 改成原函数以后，Ceres 打印的 cost 数值不再等于 Kalibr 日志里的 cost。Kalibr 报告的是 $\sum\tfrac12 w(s_k)s_k$ 这一类加权平方和，而这里报告的是 $\sum\tfrac12\int_0^{s_k}w$。两者在 $s\to0$ 时同阶（都趋近 $\tfrac12 s$），但在有明显 outlier 时数值不同。做数值对拍时应当比较**参数估计值和每项的 $w(s_k)$**，而不是比较 cost 标量。

因此，本书后续说"Cauchy width 10 已对齐"时，包含两层含义：第一，policy 类型和参数对应 Kalibr，即 $w(s)$ 逐点相同；第二，每轮线性系统与 Kalibr 的 IRLS 完全一致——如上所述，这一条对凹损失是自动成立的，$\rho_2$ 置零只是把它写明。**不含**cost 标量数值相同这一层。
