...
> **一个免费的自检。** 投影是 $\tilde{\mathbf p}_c$ 的**零次齐次函数**：把整个齐次点乘上任意 $\lambda\ne 0$，$X/Z$ 与 $Y/Z$ 都不变，于是 $\pi(\lambda\tilde{\mathbf p}_c)=\pi(\tilde{\mathbf p}_c)$。对零次齐次函数套 Euler 恒等式，立刻得到
>
> $$
> \mathbf J_{\pi,\tilde{\mathbf p}_c}\,\tilde{\mathbf p}_c=\mathbf 0,
> $$
>
> 也就是**该点自身的齐次坐标一定落在这个 Jacobian 的零空间里**。代进上面的矩阵验证第一行：
>
> $$
> \frac{f_u}{Z}\cdot X+0\cdot Y-\frac{f_uX}{Z^2}\cdot Z+0\cdot w
> =\frac{f_uX}{Z}-\frac{f_uX}{Z}=0,
> $$
>
> 第二行同理。这条恒等式对本章后面每一个 distortion 模型的 $2\times 4$ 点 Jacobian 都成立（distortion 只作用在已经归一化的 $\mathbf q$ 上，不破坏零次齐次性）。所以它是一个随手可用的检查：**手推或数值求出的 projection Jacobian，右乘该点齐次坐标若不得零，就一定推错了。**
